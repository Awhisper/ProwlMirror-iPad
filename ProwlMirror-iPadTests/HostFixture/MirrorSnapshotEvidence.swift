...
nonisolated struct MirrorSnapshotEvidence: Equatable, Sendable {
  ...
  static func read(_ frame: MirrorFrame) -> Self? {
    ...
    var reader = Reader(
      bytes: Array(frame.bytes), rows: Int(frame.rows), columns: Int(frame.columns))
    return reader.read()
  }
  ...
  private struct Reader {
    ...
    mutating func csi() -> Bool {
      ...
      guard index < bytes.count,
        let parameters = String(bytes: bytes[start..<index], encoding: .utf8)
      else {
        return false
      }
      ...
      case 72:
        ...
        guard cursor == nil, fields.count == 2, let row = Int(fields[0]),
    ...
    private func consumeColor(_ fields: [Substring], position: inout Int) -> Bool {
      ...
      guard position + count <= fields.count,
        fields[position..<(position + count)].allSatisfy({
