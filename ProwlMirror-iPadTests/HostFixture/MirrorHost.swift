...
final class MirrorHost {
  ...
  private func accept(_ connection: NWConnection) {
    guard listener != nil, peers.count < 16,
      connectionAttempts.accept(now: ProcessInfo.processInfo.systemUptime)
    else {
      connection.cancel()
      return
    }
  ...
  private func handle(_ message: MirrorMessage, from peer: MirrorConnection) {
    do {
      ...
      case .submissionStatus:
        guard message.version == 2, let key = submissionKey(message) else {
          throw MirrorProtocolError.invalidMessage
        }
        let outcome =
          key.run == hostRunID
  ...
  private func subscribe(_ message: MirrorMessage, peer: MirrorConnection) throws {
    ...
    if modern, source.supportsSubmission {
      ...
      peer.send(
        MirrorMessage(
          version: 2, kind: .state, subscriptionID: next.id, agentState: next.agentState))
    }
    if pollTask == nil {
  ...
  private func submissionReply(_ key: MirrorSubmissionLedger.Key, outcome: MirrorSubmitOutcome)
    -> MirrorMessage
  {
    MirrorMessage(
  ...
  private func submit(_ message: MirrorMessage, peer: MirrorConnection) throws {
    guard message.version == 2, source.supportsSubmission,
      let subscription = subscription(for: message, peer: peer),
      subscription.representation == .text,
    ...
    Task { [weak self, weak peer] in
      ...
      let final =
        ...
        ? MirrorSubmitOutcome(status: .unknown, detail: "The Agent did not confirm delivery.")
        : outcome
      ...
      do {
        if versions[id] == 2, source.supportsSubmission {
            ...
            || subscription.agentState?.reason != state.reason
          {
            ...
            peer.send(
              MirrorMessage(
                version: 2, kind: .state, subscriptionID: subscription.id, agentState: state))
          }
        }
