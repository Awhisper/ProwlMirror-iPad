...
nonisolated struct MirrorSubmissionLedger {
  ...
  mutating func reserve(_ key: Key, text: String) -> Reservation {
    ...
    if let entry = entries[key] {
      guard entry.hash == hash else {
        return .refused("Submission ID was already used for different text.")
      }
    ...
    entries[key] = Entry(
      hash: hash, outcome: .init(status: .pending, detail: "Delivery is pending."))
    order.append(key)
    return .reserved
  }
  ...
  func receipt(_ key: Key) -> MirrorSubmitOutcome {
    entries[key]?.outcome
      ?? .init(
        status: .unknown, detail: "No retained receipt. Check the Host before sending again.")
  }
